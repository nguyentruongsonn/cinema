...
/**
 * Fixed Booking Flow Test
 *
 * Tests the complete booking flow with corrections:
 * - Selects movie with available showtimes (ID 1-5)
 * - Uses correct order request structure
...
echo colorOutput("\n========================================\n", 'blue');
echo colorOutput("  FIXED BOOKING FLOW API TEST\n", 'blue');
echo colorOutput("========================================\n\n", 'blue');
...
if ($registerResponse['code'] !== 201 && $registerResponse['code'] !== 200) {
    echo colorOutput("  Register response: " . ($registerResponse['body']['message'] ?? 'Unknown') . "\n", 'yellow');
}
...
// FIX: Select a movie that has showtimes (movies 1-5)
$movies = $moviesResponse['body']['data'];
$movieWithShowtimes = null;

foreach ($movies as $m) {
    // Movies 1-5 have showtimes based on seeder
    if ($m['id'] <= 5) {
...
if (!$movieWithShowtimes) {
    ...
    echo "  Movies 1-5 should have showtimes. Check ShowtimeSeeder.\n";
    exit(1);
}
...
if ($showtimesResponse['code'] !== 200 || empty($showtimesResponse['body']['data'])) {
    echo colorOutput("✗ Failed to get showtimes!\n", 'red');
...
// Step 7: Create order - FIX: Use correct structure
echo colorOutput("[STEP 7] Creating order...\n", 'yellow');
$orderData = [
    'showtime_id' => $showtime['id'],
    'seat_ids' => $seatIds,
    'products' => [],  // Optional: can add food/drinks here
    // 'promotion_code' => null,  // Optional
    // 'seat_hold_id' => $holdId,  // Optional
];
...
echo "Seats booked: " . implode(', ', array_column($seatsToBook, 'seat_number')) . "\n\n";
