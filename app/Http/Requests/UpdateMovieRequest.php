...
class UpdateMovieRequest extends FormRequest
{
    ...
    {
        $movie = $this->route('movie') ?? $this->route('id');

        if (!$movie instanceof Movie) {
            $movie = Movie::whereKey($movie)->first();
        }

        return $movie instanceof Movie && (bool) $this->user()?->can('update', $movie);
    }

    public function rules(): array
    {
        $movie = $this->route('movie') ?? $this->route('id');
        $movieId = $movie instanceof Movie ? $movie->getKey() : $movie;
    ...
    {
        return [
            'title.string' => 'Movie title must be a string.',
